...
`content-plan.csv` is the output. 272 unique page ideas, each with a SERP-verified
...
- **cannibalization**: `clean`, or `CHECK: <live page>` when slug tokens overlap a
  ...
  RESHAPE, SKIP or FOLD. 92 rows carry one.
- **our_angle**: one sentence, or `NONE` where a dock utility does not honestly
  solve the problem. **35 rows say NONE and that is deliberate.** Recording where
...
- **priority**: 1 to 5, 5 highest. 47 rows are priority 5 with an honest angle.
